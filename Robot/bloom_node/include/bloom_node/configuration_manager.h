...
/// ConfigurationManager provides a tiny, thread-safe configuration store for
...
public:
	...
	/// Construct with an optional ROS node. If a node is provided, call
	/// `import_from_node()` to pull parameters into the config store.
	explicit ConfigurationManager(rclcpp::Node::SharedPtr node = nullptr);
	...
	/// Parameters will be stored using their full names as keys.
	...
	/// Set or replace a key value (string). Thread-safe.
